...
def running(tombo_pr,dna_file,treshold):
    DNA_arr,colors,CpG,GpC = DNA_reading(dna_file)
    pos,stat,read_id = reading(tombo_pr)
    df = pd.DataFrame({'base': DNA_arr})
    mean_data,read_num_arr = perread_dataframe(read_id,pos,stat,DNA_arr,df,CpG,treshold)
...
Y_return,subset_1_finished,subset_2_finished,subset_3_finished,subset_4_finished = Principle_Component_Analysis(data,2,4,loc_array)

#makes a plot of LLR on CpG sites against GpC sites
CpG_arr,GpC_arr,min_DNA_arr,data_CpGpC,value_chi_square = CpG_GpC_selection(DNA_arr,data)
